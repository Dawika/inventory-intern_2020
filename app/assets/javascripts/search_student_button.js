...
function quotation_id(index, row, element) {
    return '<a href="/somsri_invoice#/invoice/view/' + row.id + '">' + row.id + '</a>';
}

function paidAtFormatter(index, row, element) {
    if (row.paid_at != null) {
        return moment(row.paid_at).format('DD/MM/YYYY HH:mm')
    }
}

function filterCurrency(index, row, element) {
    if (!index) {
        index = 0
    }
    return index.toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,')
}

function linkToInvoiceFormatter(index, row, element) {
    var link = '<p><i class="fa fa-money" aria-hidden="true"></i>ชำระแล้ว</p>';
